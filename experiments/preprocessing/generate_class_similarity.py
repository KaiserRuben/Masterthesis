...
-----
    ...
    python experiments/preprocessing/generate_class_similarity.py \\
        configs/Exp-05/phaseA/pdq_stingray-electric_ray.yaml

    # Specify a custom output path:
    python experiments/preprocessing/generate_class_similarity.py CONFIG -o OUT.parquet

    # Print 5 evenly-spaced pool pairs along the similarity axis:
    python experiments/preprocessing/generate_class_similarity.py CONFIG --select 5
...
def score_all_samples(
    ...
    Mirrors the scoring step of :func:`~src.evolutionary.seed_generator.generate_seeds`
...
def attach_seed_pool_indices(
    ...
    Seeds are emitted by :func:`~src.evolutionary.seed_generator.generate_seeds`
...
def main() -> None:
    ...
    parser.add_argument(
        "-o", "--output", type=Path,
        default=Path("runs/Exp-05/class_similarity.parquet"),
        help="Output parquet path (default: runs/Exp-05/class_similarity.parquet)",
    )
    parser.add_argument(
        "--device",
