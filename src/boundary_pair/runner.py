...
class BoundaryPairRunner:
    ...
    def _apply_resume(
        ...
        A seed is "finished" iff its ``manifest.json`` exists and the sibling
        ``evolutionary/stats.json`` carries the ``seed_metadata`` block. The
        scan, partial cleanup, index-drift sanity check, and filter narrowing
        are shared with the evolutionary runner via
        :func:`src.common.compute_resume_filter`; only this pipeline's on-disk
        layout (manifest marker, idx read from the manifest) is supplied here.
        ...
        """
        probe = SeedDirProbe(
            iter_seed_dirs=lambda save_dir, name: (save_dir / name).glob("seed_*"),
            is_finished=lambda seed_dir: (seed_dir / "manifest.json").exists(),
            read_seed_idx=lambda seed_dir: int(
                json.loads((seed_dir / "manifest.json").read_text())["seed_idx"]
            ),
            read_metadata=_read_pair_seed_metadata,
        )
        remaining = compute_resume_filter(
            ...
            cfg.name,
            seeds,
            cfg.seeds.filter_indices,
            clean_partials=self._clean_partials,
        )
        return replace(cfg, seeds=replace(cfg.seeds, filter_indices=remaining))


def _read_pair_seed_metadata(seed_dir: Path) -> dict[str, Any] | None:
    """``seed_metadata`` for a boundary-pair seed dir.

    Reads the sibling ``evolutionary/stats.json``; returns ``None`` if that
    file is absent (the resume scanner then treats the dir as not-done). JSON
    or key errors propagate to the scanner, which catches and re-runs the seed.
    """
    stats_path = seed_dir / "evolutionary" / "stats.json"
    if not stats_path.exists():
        return None
    return json.loads(stats_path.read_text()).get("seed_metadata")
