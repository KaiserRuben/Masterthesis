...
def _create_3d_connections(
    ...
    # Connection lines - monochrome with opacity encoding severity
    ...
    for _, row in df.iterrows():
        if row["clip_a"] in clip_to_umap and row["clip_b"] in clip_to_umap:
            ...
            # Monochrome: darker = higher severity
            ratio = row["rel_delta_ade"] / max_delta
            gray_val = int(180 - ratio * 140)  # Range: 180 (light) to 40 (dark)
            line_colors.append(f"rgb({gray_val},{gray_val},{gray_val})")
...
def _create_2d_connections(
    ...
    # Connection lines - monochrome with width encoding severity
    max_delta = df["rel_delta_ade"].max()
