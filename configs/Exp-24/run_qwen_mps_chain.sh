#!/usr/bin/env bash
# ═══════════════════════════════════════════════════════════════════════════
# Exp-24 Qwen2.5-VL torch/MPS chain — sequential modality sweep on Mac.
#
# Runs joint → image_only → text_only with combined stdout/stderr captured
# to runs/Exp-24/_logs/exp24_qwen_mps_chain.log. Detaches via nohup so the
# chain survives terminal close.
#
# Usage:
#   bash configs/Exp-24/run_qwen_mps_chain.sh           # detached, returns immediately
#   tail -f runs/Exp-24/_logs/exp24_qwen_mps_chain.log  # follow live
#
# Stop early:
#   pkill -f 'run_boundary_test.py configs/Exp-24/qwen_mps'
# ═══════════════════════════════════════════════════════════════════════════
set -euo pipefail
...
LOG_FILE="$LOG_DIR/exp24_qwen_mps_chain.log"
mkdir -p "$LOG_DIR"
...
CONFIGS=(
  "configs/Exp-24/qwen_mps_joint.yaml"
  "configs/Exp-24/qwen_mps_image_only.yaml"
  "configs/Exp-24/qwen_mps_text_only.yaml"
)
